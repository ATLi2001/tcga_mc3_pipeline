...
vcfs = read.delim("vcf_non_pan12.tsv",stringsAsFactors = F)

#because there are two vcfs per patient, go by patient
patients <- unique(vcfs$patient)

#directory for ssm_data.txt files
of = "phylowgs_input/"

#directory for ssm mapping files
if(!dir.exists('non_pan12_annot/')){
  dir.create('non_pan12_annot/',showWarnings = F)
}
...
for(i in 1:length(patients)){
  ...
  if(!file.exists(paste0('non_pan12_annot/',pt,'.filtered.mapping.txt'))){
    ...
    tryCatch({
      ...
      write.table(tmp,paste0('non_pan12_annot/',pt,'.filtered.mapping.txt'),row.names = F,quote=F,sep='\t' )
