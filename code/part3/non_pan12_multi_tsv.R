...
patient_list <- read.table("all_phylowgs.txt",stringsAsFactors = FALSE,header = FALSE)
patient_list <- patient_list$V1

#leave only TCGA Barcode
patient_list <- substring(patient_list,16,31)

#cut down to only non_pan12
non_pan12 <- read.table("~/Desktop//Austin/MSTC/Capstone Project/Austin Pipeline MC3/TitanCNA/non_pan12_jobs_list.txt",
                        stringsAsFactors = FALSE,header = FALSE)
non_pan12 <- non_pan12$V1
patient_list <- intersect(patient_list,non_pan12)
...
for(i in 1:length(patient_list)){
  #--input SSM
  tsv_input <- paste("gs://austin_pipeline_mc3/phylowgs/non_pan12/INPUT/phylowgs_input",patient_list[i],
                     "ssm_data.txt", sep = "/")
  #--input CNA
  tsv_cna <- paste("gs://austin_pipeline_mc3/phylowgs/non_pan12/INPUT/phylowgs_input", patient_list[i],
                   "cnv_data.txt", sep = "/")
  #--output OUTPUT_DIR
  tsv_output <- paste("gs://austin_pipeline_mc3/phylowgs/non_pan12/OUTPUT",patient_list[i],
                      sep = "/")
...
write.table(tsv, file="non_pan12_multi.tsv",sep="\t",row.names=F,col.names = T,quote=F)
