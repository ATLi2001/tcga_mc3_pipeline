...
phylowgs_tsv <- read.delim("~/Desktop/Austin/MSTC/Capstone Project/Austin Pipeline MC3/Google Cloud/phylowgs_multi/non_pan12_multi.tsv",
                           header = FALSE,
                           stringsAsFactors = FALSE)

#pan12 metadata file to match patient to cancer type
non_pan12_meta <- read.delim("~/Desktop/Austin/MSTC/Capstone Project/Austin Pipeline MC3/TitanCNA/phylowgs_mc3-non_pan12-non.pan12.paired_bam_hg38_chr.tsv",
                         stringsAsFactors = FALSE)
...
for(i in 2:nrow(phylowgs_tsv)){
  ...
  curr_bar <- substr(phylowgs_tsv$V1[i],52,67)
  
  #identify which cancer type this patient is of
  curr_cancer <- non_pan12_meta$project_short_name[which(non_pan12_meta$sample_barcode==curr_bar)]
...
#output column
phylowgs_tsv$V3 <- "gs://austin_pipeline_mc3/phylowgs/non_pan12/summ_stats/*"
...
write.table(phylowgs_tsv,file="non_pan12_tree_reader_multi.tsv",sep = "\t",row.names = F,col.names = F,quote = F)
