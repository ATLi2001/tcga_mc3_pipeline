#manifest file names
pan12_manifest <- list.files("pan12_manifest",full.names = TRUE)
#manifest files
datalist <- lapply(pan12_manifest, FUN = read.delim,header=TRUE,stringsAsFactors=FALSE)
...
write.table(out,file = "pan12_clinical_manifest.txt",col.names = T,row.names = F,sep = '\t',quote = F)
